...
|----------|------|-------------|---------|--------|
| 🔵 Structural | `Contains` | Parent contains child | `Namespace` → `Class` | Implemented |
| 🔵 Structural | `Inherits` | Class extends base | `AdminUser` → `User` | Implemented |
| 🔵 Structural | `Implements` | Type implements interface | `UserService` → `IUserService` | Implemented |
| 🔵 Structural | `Overrides` | Method overrides base | `Derived.DoWork()` → `Base.DoWork()` | Implemented |
| 🟢 Coupling | `Calls` | Method invokes another | `CreateOrder()` → `GetUser()` | Implemented |
| 🟢 Coupling | `Instantiates` | Creates instance via `new` | `Handler` → `new Service()` | Implemented |
| 🟢 Coupling | `References` | Type/member reference | Field types, return types, parameters | Implemented |
| 🟡 DataFlow | `Reads` | Accesses field/property value | `GetName()` reads `_name` | Planned |
| 🟡 DataFlow | `Writes` | Assigns to field/property | `SetName()` writes `_name` | Planned |
| 🟡 DataFlow | `Returns` | Method returns a type | `GetUser()` returns `User` | Planned |
| 🟡 DataFlow | `Creates` | Method instantiates a type | `CreateOrder()` creates `Order` | Planned |
| 🟡 DataFlow | `Throws` | Method can throw exception | `Validate()` throws `ValidationException` | Planned |
| 🟡 DataFlow | `DataFlowsTo` | Data flows from variable A to B | Track through assignments | Planned |
